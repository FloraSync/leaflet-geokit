...
describe("leaflet-assets", () => {
  describe("injectLeafletStyles", () => {
    ...
    it("should not inject styles twice (idempotent behavior)", () => {
      // First injection
      injectLeafletStyles(shadowRoot);
      const firstStyleElement = shadowRoot.querySelector(
        "style[data-leaflet-styles]",
      );
      ...
      const styleElements = shadowRoot.querySelectorAll(
        "style[data-leaflet-styles]",
      );
    ...
    it("should append the style element to the shadow root", () => {
      ...
      expect(
        shadowRoot.lastElementChild?.getAttribute("data-leaflet-styles"),
      ).toBe("true");
    });
